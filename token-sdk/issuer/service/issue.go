...
// Issue issues an amount of tokens to a wallet. It connects to the other node, prepares the transaction,
// gets it approved by the auditor and sends it to the blockchain for endorsement and commit.
func (s TokenService) History(tokenType string, quantity uint64, recipient string, recipientNode string, message string) (txID string, err error) {
	logger.Infof("History: going to issue %d %s to [%s] on [%s] with message [%s]", quantity, tokenType, recipient, recipientNode, message)
	res, err := viewregistry.GetManager(s.FSC).InitiateView(&IssueHistoryView{
	...
	if err != nil {
		logger.Errorf("error printing issuing history: %s", err.Error())
		return
	}
	txID, ok := res.(string)
	if !ok {
		return "", errors.New("cannot parse history response")
	}
	logger.Infof("History: successfully issued %d %s to [%s] on [%s] with message [%s]. ID: [%s]", quantity, tokenType, recipient, recipientNode, message, txID)
	return
}
...
// IssueHistory contains the input information to issue a token
